...
# 修改这个函数
def get_historical_flow_fallback(target_date, fallback_flow_path, flow_cols=['进港流量']):
    """
...
def prepare_encoder_input(historical_date_str, work_dir, scalers):
    ...
    required_cols = ['进港流量'] + feature_names['other'] + feature_names['cyclic']
    ...
    y_past_scaled = scalers['target'].transform(encoder_input_df[['进港流量']])
